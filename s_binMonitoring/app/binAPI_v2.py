...
# from config import (
#     REDIS_HOST, REDIS_PORT, KAFKA_TOPIC, KAFKA_SERVER,
...
# CORS(app, resources={r"/v2/*": {"origins": "*"}}, supports_credentials=True)
...
#             if topic:
#                 return jsonify({sensor_id: topic.decode()}), 200
#             return jsonify({"error": "Sensor not found"}), 404

#         keys = r.keys("*")
#         sensors = {key.decode(): r.get(key).decode() for key in keys}
...
logger = logging.getLogger("binmonitoringAPI")

# App init
app = FastAPI(root_path="/v2/binmonitoring")
...
try:
    r = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, decode_responses=True)
    r.ping()
    print("✅ Redis ligado com sucesso.")
except redis.RedisError as e:
    print(f"❌ Erro ao conectar ao Redis: {e}")
    exit(1)
...
try:
    ...
    print(f"❌ Erro ao iniciar Kafka Producer: {e}")
    exit(1)
...
class DeleteSensorsModel(BaseModel):
...
class DeleteTopicsModel(BaseModel):
...
# Routes
@app.post("/sensors", status_code=201, tags=["Sensor"])
def add_sensor(sensor: SensorModel):
    if not validate_name(sensor.sensor_id) or not validate_name(sensor.topic):
        raise HTTPException(status_code=400, detail="Invalid format")
    if r.exists(sensor.sensor_id):
        raise HTTPException(status_code=409, detail="Sensor already exists")
    r.set(sensor.sensor_id, sensor.topic)
    return {"message": f"Sensor {sensor.sensor_id} added to topic {sensor.topic}"}

@app.get("/sensors", tags=["Sensor"])
def list_sensors():
    try:
        sensors = {k: r.get(k) for k in r.keys("*")}
        return sensors
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/sensors", tags=["Sensor"])
def remove_sensors(req: DeleteSensorsModel):
    if req.all:
        keys = r.keys("*")
        for key in keys:
            r.delete(key)
        return {"message": f"All sensors removed ({len(keys)})"}
    if not req.sensor_ids:
        raise HTTPException(status_code=400, detail="Provide sensor_ids or set all=true")
    removed = [sid for sid in req.sensor_ids if r.exists(sid)]
    for sid in removed:
        r.delete(sid)
    return {"message": f"Sensors removed: {removed}"}

@app.post("/topic", status_code=201, tags=["Topics"])
def create_topic(t: TopicModel):
    if not validate_name(t.topic):
        raise HTTPException(status_code=400, detail="Invalid topic name")
    try:
        admin = KafkaAdminClient(bootstrap_servers=KAFKA_SERVER, client_id="bin_api_admin")
        admin.create_topics([NewTopic(name=t.topic, num_partitions=3, replication_factor=1)])
        return {"message": f"Topic {t.topic} created"}
    except TopicAlreadyExistsError:
        raise HTTPException(status_code=409, detail="Topic already exists")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/topic", tags=["Topics"])
def list_topics():
    admin = KafkaAdminClient(bootstrap_servers=KAFKA_SERVER, client_id="bin_api_admin")
    return {"topics": admin.list_topics()}

@app.delete("/topic", tags=["Topics"])
def delete_topics(req: DeleteTopicsModel):
    failed = []
    deleted = []
    try:
        admin = KafkaAdminClient(bootstrap_servers=KAFKA_SERVER, client_id="bin_api_admin")
...
def shutdown_handler(sig, frame):
    logger.info("A encerrar conexões...")
    try:
        producer.close()
        r.close()
    except Exception as e:
