...
# App init
app = FastAPI(root_path="/v2/routes")

# CORS
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Ajusta conforme segurança
...
class CoordinatesModel(BaseModel):
    ...
    mode: str = Field(..., description="Modo de transporte", example="car")
    truck_id: Optional[str] = Field(None, description="ID do caminhão (se modo for 'car')")
...
# UTIL
def call_osrm(start, waypoints, end, mode):
    try:
        if mode == "car":
            all_coords = [start] + waypoints + [end]
            coords_str = ";".join([f"{lon},{lat}" for lat, lon in all_coords])
            url = f"{OSRM_BASE_URL}/trip/v1/driving/{coords_str}?source=first&destination=last&overview=full&geometries=geojson"
        else:
            coords_str = f"{start[1]},{start[0]};{end[1]},{end[0]}"
            url = f"{OSRM_BASE_URL}/route/v1/foot/{coords_str}?overview=full&geometries=geojson"

        resp = requests.get(url, timeout=5)
        if resp.status_code != 200:
            raise HTTPException(status_code=503, detail="Erro no serviço OSRM")

        data = resp.json()
        route = data.get("trips" if mode == "car" else "routes", [None])[0]
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="Erro ao comunicar com OSRM")

# ENDPOINTS
@app.post("/", response_model=RouteResponse, tags=["routes"])
def create_route(payload: CoordinatesModel):
    coords = payload.coordinates
    mode = payload.mode

    if len(coords) < 2:
        raise HTTPException(status_code=400, detail="Mínimo de 2 coordenadas necessário")

    if mode not in ["car", "walking"]:
        raise HTTPException(status_code=400, detail="Modo inválido. Use 'car' ou 'walking'.")

    start, *waypoints, end = coords
    route_result = call_osrm(start, waypoints, end, mode)
    ...
    if mode == "car":
        if not payload.truck_id:
            raise HTTPException(status_code=400, detail="truck_id é obrigatório para modo 'car'")
        db_result = sc.create_route(route_id, route_result["route_coordinates"],
...
def get_routes(
    ...
    year: Optional[int] = Query(None, ge=2000, le=datetime.now().year),
    truck_id: Optional[str] = Query(None)
):
    if id:
        route = sc.get_route(id)
        if "error" in route:
            if route["error"] == "Route not found":
                raise HTTPException(status_code=404, detail="Rota não encontrada")
            raise HTTPException(status_code=503, detail="Erro no acesso ao banco de dados")
        return route

    routes = sc.get_routes_history(day, month, year, truck_id)
    if "error" in routes:
        if routes["error"] == "No routes found":
            raise HTTPException(status_code=404, detail="Nenhuma rota encontrada com esses critérios")
        raise HTTPException(status_code=503, detail="Erro no banco de dados")
    return routes

@app.delete("/", tags=["routes"])
def delete_route(id: str = Query(..., description="ID da rota")):
    result = sc.delete_route(id)
    if "error" in result:
        if result["error"] == "Route not found":
            raise HTTPException(status_code=404, detail="Rota não encontrada")
        raise HTTPException(status_code=503, detail="Erro ao excluir a rota")
    return {"message": f"Rota '{id}' excluída com sucesso"}
